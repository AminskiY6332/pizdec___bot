...
if REDIS is not None:
    try:
        ...
        logger.warning(f"Ошибка инициализации Redis клиента: {e}")
        redis_client = None

# Создаем кеши только если Redis доступен
if redis_client is not None:
    user_cache = RedisUserCache(redis_client)
    active_model_cache = RedisActiveModelCache(redis_client)
    gen_params_cache = RedisGenParamsCache(redis_client)
else:
    user_cache = None
    active_model_cache = None
    gen_params_cache = None
        ...
        async def wrapper(*args, **kwargs):
            ...
            if user_id and user_cache is not None:
                try:
                    ...
                    logger.warning(f"Ошибка при удалении кеша для user_id={user_id}: {e}")
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            if 'utm_source' not in columns:
                await c.execute("ALTER TABLE users ADD COLUMN utm_source TEXT DEFAULT NULL")
                logger.info("Добавлен столбец utm_source в таблицу users")
            ...
            # Проверка и добавление столбцов style и ratio в user_actions
            await c.execute("PRAGMA table_info(user_actions)")
            user_actions_columns = [col[1] for col in await c.fetchall()]
            if 'style' not in user_actions_columns:
                await c.execute("ALTER TABLE user_actions ADD COLUMN style TEXT DEFAULT NULL")
                logger.info("Добавлен столбец style в таблицу user_actions")
            if 'ratio' not in user_actions_columns:
                await c.execute("ALTER TABLE user_actions ADD COLUMN ratio TEXT DEFAULT NULL")
                logger.info("Добавлен столбец ratio в таблицу user_actions")

            # Проверка и добавление столбцов style и ratio в generation_log
            await c.execute("PRAGMA table_info(generation_log)")
            generation_log_columns = [col[1] for col in await c.fetchall()]
            if 'style' not in generation_log_columns:
                await c.execute("ALTER TABLE generation_log ADD COLUMN style TEXT DEFAULT NULL")
                logger.info("Добавлен столбец style в таблицу generation_log")
            if 'ratio' not in generation_log_columns:
                await c.execute("ALTER TABLE generation_log ADD COLUMN ratio TEXT DEFAULT NULL")
                logger.info("Добавлен столбец ratio в таблицу generation_log")
            ...
            # ИСПРАВЛЕНО: убран автоматический бэкап при инициализации БД для предотвращения аномальной частоты
    except Exception as e:
...
async def verify_backup_integrity(backup_path: str) -> bool:
    ...
    try:
        async with aiosqlite.connect(backup_path) as backup_conn:
            ...
            if is_valid:
                # Дополнительная проверка - проверяем, что основные таблицы существуют
                cursor = await backup_conn.execute("SELECT name FROM sqlite_master WHERE type='table'")
                tables = await cursor.fetchall()
                required_tables = {'users', 'user_ratings', 'payments'}
                existing_tables = {table[0] for table in tables}
                has_required_tables = required_tables.issubset(existing_tables)

                logger.info(f"Backup integrity check: {backup_path} - OK, tables: {len(existing_tables)}")
        ...
        logger.error(f"Error verifying backup integrity {backup_path}: {e}", exc_info=True)
        return False

async def backup_database() -> bool:
    """Безопасное создание резервной копии базы данных с использованием SQLite BACKUP API"""
    ...
    try:
        ...
        # Безопасное копирование с использованием SQLite BACKUP API
        async with aiosqlite.connect(DATABASE_PATH) as source_conn:
            # Создаем временное соединение для бэкапа
            async with aiosqlite.connect(backup_path) as backup_conn:
        ...
        if not await verify_backup_integrity(backup_path):
            logger.error(f"Backup integrity verification failed, removing corrupted backup: {backup_path}")
            if os.path.exists(backup_path):
                os.remove(backup_path)
            return False

        logger.info(f"✅ Safe database backup created and verified: {backup_path}")
        ...
        logger.error(f"❌ Error creating safe database backup: {e}", exc_info=True)
        # Удаляем поврежденный бэкап если он был создан
        if backup_path and os.path.exists(backup_path):
            try:
                os.remove(backup_path)
                logger.info(f"Removed corrupted backup file: {backup_path}")
            except Exception as cleanup_error:
                logger.error(f"Failed to remove corrupted backup: {cleanup_error}")
        return False
    ...
    try:
        ...
        if len(backups) > 7:
            for old_backup in backups[:-7]:
                ...
                logger.info(f"🗑️ Old backup removed: {old_backup}")
    except Exception as e:
        logger.error(f"Error cleaning up old backups: {e}", exc_info=True)

async def periodic_backup():
    """Периодическое создание резервных копий БД с улучшенной обработкой ошибок"""
    logger.info(f"📅 Scheduled backup started, interval: {BACKUP_INTERVAL_HOURS} hours")
    ...
    while True:
        try:
            ...
            if backup_success:
                ...
                # Уведомляем админов об успешном бэкапе (можно добавить позже)
            else:
                logger.error("❌ Scheduled backup failed")
                # Уведомляем админов об ошибке (можно добавить позже)

        except asyncio.CancelledError:
            logger.info("🛑 Periodic backup task cancelled")
            break
        except Exception as e:
            logger.error(f"❌ Critical error in periodic_backup: {e}", exc_info=True)
...
async def get_backup_status() -> dict:
    ...
    try:
        ...
        latest_size = os.path.getsize(latest_backup)
        latest_time = os.path.getmtime(latest_backup)

        # Проверяем целостность последнего бэкапа
        integrity_ok = await verify_backup_integrity(latest_backup)

        return {
            "status": "ok" if integrity_ok else "error",
            "total_backups": len(backups),
            "latest_backup": backups[-1],
            "latest_size_mb": round(latest_size / 1024 / 1024, 1),
            "latest_time": datetime.fromtimestamp(latest_time).strftime('%Y-%m-%d %H:%M:%S'),
...
def start_periodic_tasks():
    ...
    if BACKUP_ENABLED:
        ...
        logger.warning("⚠️ Автоматические бэкапы отключены (BACKUP_ENABLED=False)")
...
@invalidate_cache()
async def add_user_without_subscription(user_id: int, username: str, first_name: str, referrer_id: Optional[int] = None, utm_source: Optional[str] = None) -> None:
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH, timeout=10) as conn:
            ...
            await c.execute("SELECT referrer_id, created_at, utm_source FROM users WHERE user_id = ?", (user_id,))
            ...
            if existing_user_data:
                ...
                current_utm_source = existing_user_data[2]

                # Обновляем только если utm_source не установлен или отличается
                utm_update_needed = current_utm_source is None and utm_source is not None

                if current_referrer_id is None and referrer_id is not None:
                    if utm_update_needed:
                        await c.execute(
                            """UPDATE users
                               SET username = ?, first_name = ?, referrer_id = ?, utm_source = ?, updated_at = ?
                               WHERE user_id = ?""",
                            (username, first_name, referrer_id, utm_source, current_timestamp, user_id)
                        )
                        logger.info(f"Данные пользователя user_id={user_id} обновлены. Новый referrer_id={referrer_id}, utm_source={utm_source}.")
                    else:
                        await c.execute(
                            """UPDATE users
                    ...
                    if utm_update_needed:
                        await c.execute(
                            """UPDATE users
                               SET username = ?, first_name = ?, utm_source = ?, updated_at = ?
                               WHERE user_id = ?""",
                            (username, first_name, utm_source, current_timestamp, user_id)
                        )
                        logger.info(f"Данные пользователя user_id={user_id} обновлены (referrer_id не изменен: {current_referrer_id}), utm_source={utm_source}.")
                    else:
                        await c.execute(
                            """UPDATE users
                ...
                await c.execute(
                    '''INSERT INTO users (
                        ...
                        is_notified, first_purchase, referrer_id, utm_source, created_at, updated_at,
                        welcome_message_sent
                    ) VALUES (?, ?, ?, 0, 0, 0, 1, ?, ?, ?, ?, 0)''',
                    (user_id, username, first_name, referrer_id, utm_source, current_timestamp, current_timestamp)
                )
                logger.info(f"Пользователь user_id={user_id} добавлен с referrer_id={referrer_id}, utm_source={utm_source}.")
        ...
        if not existing_user_data:
            await log_user_action(user_id, 'start_bot', {
                'referrer_id': referrer_id,
                'utm_source': utm_source
            })
            if referrer_id and referrer_id != user_id and referrer_exists:
                await log_user_action(user_id, 'use_referral', {
                    'referrer_id': referrer_id,
                    'utm_source': utm_source
                })
...
async def get_user_by_username(username: str) -> Optional[Dict[str, Any]]:
    """Получает полную информацию о пользователе по username"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
        ...
        logger.error(f"Ошибка получения информации о пользователе по username {username}: {e}", exc_info=True)
        return None
        ...
        async with aiosqlite.connect(DATABASE_PATH, timeout=15) as conn:
            ...
            if user_cache is not None:
                try:
                    ...
                    logger.warning(f"Ошибка удаления кеша для referrer_id={referrer_id}: {e}")
            return True
...
async def check_database_user(user_id: int) -> Tuple[int, int, int, Optional[str], int, int, Optional[str], Optional[int], Optional[str], int, Optional[str], int, Optional[str], Optional[str]]:
    ...
    if user_cache is not None:
        try:
            ...
            logger.warning(f"Ошибка получения кеша для user_id={user_id}: {e}")
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
            ...
            if result:
                ...
                if user_cache is not None:
                    try:
                        ...
                        logger.warning(f"Ошибка сохранения кеша для user_id={user_id}: {e}")
...
async def add_resources_on_payment(user_id: int, plan_key: str, payment_amount: float, payment_id_yookassa: str, bot: Bot = None, is_first_purchase: bool = None) -> bool:
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH, timeout=15) as conn:
            ...
            # Обновляем статистику платежей после завершения основной транзакции
            try:
                await update_user_payment_stats(user_id, payment_amount)
            except Exception as e:
                logger.warning(f"Не удалось обновить статистику платежей для user_id={user_id}: {e}")
            ...
            if bot:
                logger.info(f"🔔 Начинаем отправку уведомления пользователю {user_id} о платеже")
                try:
                    ...
                    logger.info(f"🔔 Отправляем уведомление пользователю {user_id}: {message_text[:100]}...")
                    ...
                    logger.info(f"✅ Уведомление пользователю {user_id} отправлено успешно")
                except Exception as e:
                    logger.error(f"❌ Ошибка отправки уведомления пользователю {user_id}: {e}", exc_info=True)
            else:
                logger.warning(f"⚠️ Bot instance не передан, уведомление пользователю {user_id} НЕ отправлено")
...
async def log_generation(user_id: int, generation_type: str, replicate_model_id: str, units_generated: int, style: str = None, ratio: str = None):
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            await c.execute('''INSERT INTO generation_log (
                               ...
                               cost_per_unit, total_cost, style, ratio
                           ) VALUES (?, ?, ?, ?, ?, ?, ?, ?)''',
                           (user_id, generation_type, replicate_model_id,
                            units_generated, float(cost_per_unit), float(total_cost), style, ratio))
...
async def is_user_blocked(user_id: int) -> bool:
    ...
    try:
        ...
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            if result and user_cache is not None:
                try:
                    await check_database_user(user_id)  # Обновляет кэш
                except Exception as e:
                    logger.warning(f"Ошибка обновления кеша для user_id={user_id}: {e}")
...
async def get_payments_by_date(start_date: Optional[str] = None, end_date: Optional[str] = None) -> List[Tuple]:
    ...
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            query = """
                SELECT p.user_id, p.plan, p.amount, p.status, p.created_at,
                       p.payment_id, u.username, u.first_name
                FROM payments p
        ...
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            ...
            if result:
                ...
                if 'Z' in created_at_str:
                    ...
                    user_created_at = datetime.fromisoformat(created_at_str + '+00:00')

                # Создаем cutoff_datetime с timezone
                cutoff_datetime = datetime.fromisoformat(cutoff_date + " 00:00:00+00:00")
                return user_created_at < cutoff_datetime

            return False
    except Exception as e:
        logger.error(f"Ошибка проверки возраста пользователя {user_id}: {e}", exc_info=True)
        return False

@invalidate_cache()
async def update_user_utm_source(user_id: int, utm_source: str) -> bool:
    """Обновляет UTM источник для пользователя"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            await conn.execute(
                "UPDATE users SET utm_source = ?, updated_at = CURRENT_TIMESTAMP WHERE user_id = ?",
                (utm_source, user_id)
            )
            await conn.commit()
        logger.info(f"UTM источник обновлен для user_id={user_id}: {utm_source}")
        return True
    except Exception as e:
        logger.error(f"Ошибка обновления UTM источника для user_id={user_id}: {e}", exc_info=True)
        return False

async def get_user_utm_source(user_id: int) -> Optional[str]:
    """Получает UTM источник пользователя"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
            await c.execute(
                "SELECT utm_source FROM users WHERE user_id = ?",
                (user_id,)
            )
            result = await c.fetchone()
            return result['utm_source'] if result else None
    except Exception as e:
        logger.error(f"Ошибка получения UTM источника для user_id={user_id}: {e}", exc_info=True)
        return None

@retry_on_locked(max_attempts=10, initial_delay=0.5)
async def log_user_action_with_style_ratio(user_id: int, action: str, details: Dict[str, Any] = None,
                                          style: Optional[str] = None, ratio: Optional[str] = None, conn=None):
    """Логирует действие пользователя с дополнительными полями style и ratio"""
    try:
        if conn is None:
            async with aiosqlite.connect(DATABASE_PATH, timeout=10) as conn:
                await conn.execute("PRAGMA busy_timeout = 10000")
                ...
                await c.execute(
                    '''INSERT INTO user_actions
                       (user_id, action, details, style, ratio, created_at)
                       VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)''',
                    (user_id, action, details_json, style, ratio)
                )
                await conn.commit()
        else:
            c = await conn.cursor()
        ...
        logger.debug(f"Действие пользователя с style/ratio записано: user_id={user_id}, action={action}, style={style}, ratio={ratio} ✅")
    except aiosqlite.OperationalError as e:
        logger.error(f"Ошибка логирования действия с style/ratio user_id={user_id}: {e} 🚫")
        raise
    except Exception as e:
        logger.error(f"Неизвестная ошибка логирования действия с style/ratio user_id={user_id}: {e} 🚫", exc_info=True)
        raise

async def get_user_actions_with_style_ratio(user_id: Optional[int] = None,
                                           action: Optional[str] = None,
                                           style: Optional[str] = None,
                                           ratio: Optional[str] = None,
                                           start_date: Optional[datetime] = None,
                                           end_date: Optional[datetime] = None) -> List[Dict[str, Any]]:
    """Получает статистику действий пользователей с фильтрацией по style и ratio"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
        ...
        logger.error(f"Ошибка получения статистики действий с style/ratio: {e}", exc_info=True)
        return []

async def get_users_by_utm_source(utm_source: str) -> List[Dict[str, Any]]:
    """Получает всех пользователей с указанным UTM источником"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
            await c.execute(
                """SELECT user_id, username, first_name, email, created_at, utm_source
                   FROM users
                   WHERE utm_source = ?
                   ORDER BY created_at DESC""",
                (utm_source,)
        ...
        logger.error(f"Ошибка получения пользователей по UTM источнику {utm_source}: {e}", exc_info=True)
        return []

async def get_utm_source_statistics() -> Dict[str, int]:
    """Получает статистику по UTM источникам"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()
            ...
            rows = await c.fetchall()
            return {row['utm_source']: row['count'] for row in rows}
    except Exception as e:
        logger.error(f"Ошибка получения статистики UTM источников: {e}", exc_info=True)
        return {}

async def get_style_ratio_statistics(start_date: Optional[datetime] = None,
                                   end_date: Optional[datetime] = None) -> Dict[str, Dict[str, int]]:
    """Получает статистику использования стилей и соотношений сторон"""
    try:
        async with aiosqlite.connect(DATABASE_PATH) as conn:
            conn.row_factory = aiosqlite.Row
            c = await conn.cursor()

            query = """SELECT style, ratio, COUNT(*) as count
                      FROM user_actions
                      WHERE style IS NOT NULL OR ratio IS NOT NULL"""
            params = []
            ...
            query += " GROUP BY style, ratio ORDER BY count DESC"
            ...
            for row in rows:
                if row['style']:
                    style_stats[row['style']] = style_stats.get(row['style'], 0) + row['count']
                if row['ratio']:
                    ratio_stats[row['ratio']] = ratio_stats.get(row['ratio'], 0) + row['count']
        ...
        logger.error(f"Ошибка получения статистики стилей и соотношений: {e}", exc_info=True)
...
async def get_total_users_count() -> int:
    ...
    try:
        ...
        logger.error(f"Ошибка получения общего количества пользователей: {e}", exc_info=True)
        return 0
