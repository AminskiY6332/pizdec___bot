...
from handlers.utils import safe_escape_markdown as escape_md, send_message_with_fallback, create_payment_link, get_tariff_text, safe_escape_markdown, anti_spam
...
from database import (
    ...
    log_generation, search_users_by_query, is_user_blocked, delete_user_activity, block_user_access
)
    ...
    for char in invalid_chars:
        if char in email:
            ...
            await send_message_with_fallback(
                bot,
                user_id,
                safe_escape_markdown("❌ Email содержит недопустимые символы. Используйте только буквы, цифры, точку, дефис и @.", version=2),
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 Назад к пакетам", callback_data="subscribe")]]),
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
