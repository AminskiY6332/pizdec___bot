...
async def handle_aspect_ratio_callback(query: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        if current_style_set == 'new_male_avatar':
            ...
            # TODO: Add GENERATION_STYLES import if needed
...
async def handle_confirm_generation_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    ...
    try:
        if generation_type in ['with_avatar', 'photo_to_photo']:
            await generate_image(query.message, state, 2, user_id)
        elif generation_type == 'ai_video_v2_1':
...
async def handle_subscribe_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    ...
    keyboard = []
    # Фильтруем тарифы: убираем admin_premium и dev_only (кроме главного разработчика)
    available_tariffs = {}
    for k, v in TARIFFS.items():
        if k == "admin_premium":
            continue
        if v.get("dev_only") and user_id != 5667999089:  # Главный разработчик
            continue
    ...
    try:
        ...
        if email:
            ...
            try:
                ...
                # Планируем проверку статуса платежа через 10 минут
                from datetime import datetime, timedelta
                import pytz
                check_time = datetime.now(pytz.timezone('Europe/Moscow')) + timedelta(minutes=10)
                ...
                try:
                    ...
                    if scheduler_instance:
                        scheduler_instance.add_job(
                            ...
                            trigger='date',
                            run_date=check_time,
                            args=[query.bot, user_id, payment_id, payment_message.message_id, tariff_key],
                            id=f"payment_check_{payment_id}_{user_id}",
                            misfire_grace_time=300
                        )
                        logger.info(f"Запланирована проверка платежа {payment_id} для user_id={user_id} в {check_time}")
                except Exception as e:
                    logger.warning(f"Не удалось запланировать проверку платежа: {e}")
            except Exception as e:
...
async def handle_start_training_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    """Начало обучения аватара."""
    logger.debug(f"handle_start_training_callback вызван для user_id={user_id}")
    try:
        user_data = await state.get_data()
        training_photos = user_data.get('training_photos', [])
        photo_count = len(training_photos)
        logger.debug(f"Количество фото для обучения user_id={user_id}: {photo_count}")
        if photo_count < 10:
            await safe_answer_callback(query, f"❌ Недостаточно фото. Нужно минимум 10, у вас {photo_count}", show_alert=True)
            await state.clear()
            text = escape_message_parts(
                f"❌ Недостаточно фотографий для обучения аватара.",
                f" Загружено: {photo_count}, нужно минимум: 10.",
                version=2
            )
            logger.debug(f"handle_start_training_callback: сформирован текст: {text[:200]}...")
            await query.message.answer(
                text,
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="🔙 К загрузке фото", callback_data="continue_upload")]
            ...
            return
        await state.clear()
        await state.update_data(training_step='training', user_id=user_id)
        text = escape_message_parts(
            f"🎯 Начинаю обучение аватара с {photo_count} фотографиями...",
            " Это займет 15-20 минут. Я сообщу, когда обучение завершится.",
            version=2
        )
        logger.debug(f"handle_start_training_callback: сформирован текст: {text[:200]}...")
        await query.message.answer(
        ...
        await state.clear()
        await safe_answer_callback(query, "❌ Ошибка при запуске обучения", show_alert=True)
        text = escape_message_parts(
            "❌ Произошла ошибка при запуске обучения аватара.",
            " Попробуйте снова или обратитесь в поддержку: @AXIDI_Help",
            version=2
        )
        logger.debug(f"handle_start_training_callback: сформирован текст: {text[:200]}...")
...
async def handle_support_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    ...
    logger.debug(f"handle_support_callback вызван для user_id={user_id}")
    await delete_all_videos(state, user_id, query.bot)
    await state.clear()
    await reset_generation_context(state, "support", user_id=user_id)
    text = escape_message_parts(
        "💬 Поддержка\n\n",
        "Если у вас есть вопросы или проблемы, обращайтесь к нам:\n\n",
        "📧 Email: support@axidi.ai\n",
        "💬 Telegram: @AXIDI_Help\n",
        "🌐 Сайт: https://axidi.ai\n\n",
        "Мы ответим вам в течение 24 часов.",
        version=2
    )
    logger.debug(f"handle_support_callback: сформирован текст: {text[:200]}...")
    await query.message.answer(
        text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def handle_help_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    """Показ справки."""
    logger.debug(f"handle_help_callback вызван для user_id={user_id}")
    await delete_all_videos(state, user_id, query.bot)
    await state.clear()
    await reset_generation_context(state, "help", user_id=user_id)
    text_parts = [
        "❓ Справка\n\n",
        "🤖 Как пользоваться ботом:\n\n",
        "1️⃣ Создайте аватар в Личном кабинете\n",
        "2️⃣ Выберите тип генерации в главном меню\n",
        "3️⃣ Выберите стиль или введите свой промпт\n",
        "4️⃣ Загрузите фото (если требуется)\n",
        "5️⃣ Получите результат!\n\n",
        "💡 Советы:\n",
        "• Используйте качественные фотографии\n",
        "• Экспериментируйте с разными стилями\n",
        "• Пополняйте баланс для новых генераций\n\n",
    ...
    logger.debug(f"handle_help_callback: сформировано {len(messages)} сообщений для user_id={user_id}")
    ...
    try:
        for i, message_text in enumerate(messages):
            ...
            logger.debug(f"handle_help_callback: отправка части {i+1}/{len(messages)}, длина={len(text)}")
        logger.info(f"Справка отправлена для user_id={user_id}")
    except Exception as e:
        logger.error(f"Ошибка отправки справки для user_id={user_id}: {e}", exc_info=True)
        text = escape_message_parts(
...
async def handle_terms_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    """Показ пользовательского соглашения."""
    logger.debug(f"handle_terms_callback вызван для user_id={user_id}")
    await delete_all_videos(state, user_id, query.bot)
    await state.clear()
    await reset_generation_context(state, "terms", user_id=user_id)
    text = escape_message_parts(
        "📄 Пользовательское соглашение\n\n",
        "Полный текст пользовательского соглашения доступен по ссылке:\n",
        "https://telegra.ph/Polzovatelskoe-soglashenie-07-26-12\n\n",
        "Используя бота, вы соглашаетесь с условиями соглашения.",
        version=2
    )
    logger.debug(f"handle_terms_callback: сформирован текст: {text[:200]}...")
    await query.message.answer(
        text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📄 Открыть соглашение", url="https://telegra.ph/Polzovatelskoe-soglashenie-07-26-12")],
            [InlineKeyboardButton(text="🔙 В меню", callback_data="back_to_menu")]
        ]),
        parse_mode=ParseMode.MARKDOWN_V2
    )
...
# Removed duplicate function definitions
    """Показ выбора стилей."""
    user_id = query.from_user.id
    logger.debug(f"handle_style_selection_callback вызван для user_id={user_id}")
    try:
        user_data = await state.get_data()
        is_admin_generation = user_data.get('is_admin_generation', False)
        target_user_id = user_data.get('admin_generation_for_user', user_id)
        ...
        await state.update_data(
            generation_type=generation_type,
            model_key=model_key,
            current_style_set='generic_avatar',
            selected_gender='generic'
        )
        ...
        if not check_style_config('generic_avatar'):
            ...
            await state.clear()
            await query.message.answer(
                escape_md("❌ Ошибка конфигурации стилей. Обратитесь к администратору.", version=2),
                reply_markup=await create_main_menu_keyboard(user_id),
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return

        # TODO: Добавить импорт create_generic_avatar_styles_keyboard или заменить на inline keyboard
        keyboard = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🎨 Стиль 1", callback_data="style_generic_1")],
            [InlineKeyboardButton(text="🎨 Стиль 2", callback_data="style_generic_2")],
        ...
        try:
            ...
            logger.debug(f"Клавиатура общих стилей обновлена для user_id={user_id}, target_user_id={target_user_id}")
        except Exception as e:
            logger.error(f"Ошибка при обновлении клавиатуры общих стилей для user_id={user_id}: {e}", exc_info=True)
            await query.message.answer(
                text, reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN_V2
            )
        await state.set_state(BotStates.AWAITING_STYLE_SELECTION)
...
async def handle_select_new_male_avatar_styles_callback(query: CallbackQuery, state: FSMContext) -> None:
    """Показ мужских стилей."""
    user_id = query.from_user.id
    logger.debug(f"handle_select_new_male_avatar_styles_callback вызван для user_id={user_id}")
    try:
        user_data = await state.get_data()
        is_admin_generation = user_data.get('is_admin_generation', False)
        target_user_id = user_data.get('admin_generation_for_user', user_id)
        ...
        keyboard = await create_new_male_avatar_styles_keyboard(1)
        text = escape_md(f"👨 Выбери мужской стиль или введи свой промпт{' для пользователя ID ' + str(target_user_id) if is_admin_generation else ''}:", version=2)
        try:
            await query.answer()
            ...
            logger.debug(f"Клавиатура мужских стилей обновлена для user_id={user_id}, target_user_id={target_user_id}")
        except Exception as e:
            logger.error(f"Ошибка при обновлении клавиатуры мужских стилей для user_id={user_id}: {e}", exc_info=True)
            await query.message.answer(
                text, reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN_V2
        ...
        logger.error(f"Ошибка в handle_select_new_male_avatar_styles_callback для user_id={user_id}: {e}", exc_info=True)
        await state.clear()
        await query.answer("❌ Произошла ошибка", show_alert=True)
        await query.message.answer(
            escape_md("❌ Произошла ошибка. Попробуйте снова или обратитесь в поддержку: @AXIDI_Help", version=2),
...
async def handle_select_new_female_avatar_styles_callback(query: CallbackQuery, state: FSMContext) -> None:
    """Показ женских стилей."""
    user_id = query.from_user.id
    logger.debug(f"handle_select_new_female_avatar_styles_callback вызван для user_id={user_id}")
    try:
        user_data = await state.get_data()
        is_admin_generation = user_data.get('is_admin_generation', False)
        target_user_id = user_data.get('admin_generation_for_user', user_id)
        ...
        keyboard = await create_new_female_avatar_styles_keyboard(1)
        text = escape_md(f"👩 Выбери женский стиль или введи свой промпт{' для пользователя ID ' + str(target_user_id) if is_admin_generation else ''}:", version=2)
        try:
            await query.answer()
            ...
            logger.debug(f"Клавиатура женских стилей обновлена для user_id={user_id}, target_user_id={target_user_id}")
        except Exception as e:
            logger.error(f"Ошибка при обновлении клавиатуры женских стилей для user_id={user_id}: {e}", exc_info=True)
            await query.message.answer(
                text, reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN_V2
        ...
        logger.error(f"Ошибка в handle_select_new_female_avatar_styles_callback для user_id={user_id}: {e}", exc_info=True)
        await state.clear()
        await query.answer("❌ Произошла ошибка", show_alert=True)
        await query.message.answer(
            escape_md("❌ Произошла ошибка. Попробуйте снова или обратитесь в поддержку: @AXIDI_Help", version=2),
...
async def handle_use_suggested_trigger_callback(query: CallbackQuery, state: FSMContext, user_id: int, callback_data: str) -> None:
    ...
    # TODO: Реализовать handle_trigger_word_input или заменить на существующую логику
    logger.warning(f"handle_trigger_word_input не реализована для user_id={user_id}, trigger_word={trigger_word}")
    await query.answer("❌ Функция пока недоступна", show_alert=True)
