...
Перенесено из handlers/messages.py:
- handle_text() - основная обработка текстовых сообщений
- handle_photo() - обработка фотографий
- handle_email_input() - обработка ввода email
- handle_email_change_input() - обработка изменения email
'''
...
from database import is_user_blocked, block_user_access, check_database_user
from keyboards import create_main_menu_keyboard, create_subscription_keyboard, create_user_profile_keyboard, create_back_keyboard, create_aspect_ratio_keyboard, create_photo_generate_menu_keyboard, create_admin_keyboard
from generation.videos import create_video_photo_keyboard
from llama_helper import generate_assisted_prompt
from handlers.utils import safe_escape_markdown as escape_md, send_message_with_fallback, create_payment_link
...
async def handle_text(message: Message, state: FSMContext) -> None:
    ...
    # Проверяем, заблокирован ли пользователь, и автоматически разблокируем если он снова пишет
    if await is_user_blocked(user_id):
        logger.info(f'Заблокированный пользователь user_id={user_id} снова написал боту - автоматически разблокируем')
        try:
            await block_user_access(user_id, block=False, block_reason='Автоматическая разблокировка: пользователь снова написал боту')
            logger.info(f'Пользователь user_id={user_id} успешно разблокирован')

            # Отправляем приветственное сообщение о восстановлении доступа
            await send_message_with_fallback(
                bot, user_id,
                escape_md('🎉 Добро пожаловать обратно! Ваш доступ к боту восстановлен.\n\nВыберите действие:', version=2),
                reply_markup=await create_main_menu_keyboard(user_id),
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
        except Exception as e:
            logger.error(f'Ошибка при разблокировке пользователя user_id={user_id}: {e}')
            return
...
async def handle_email_input(message: Message, state: FSMContext, email: str = None) -> None:
    ...
    if not payment_amount or not payment_description or not tariff_key:
        ...
        await send_message_with_fallback(
            bot,
            user_id,
            escape_md('❌ Ошибка: данные платежа отсутствуют. Начните заново.', version=2),
    ...
    # Дополнительная валидация для YooKassa
    invalid_chars = ['...', '..', '…', ' ']  # Символы, недопустимые в YooKassa
    for char in invalid_chars:
        if char in email:
            logger.warning(f'Email содержит недопустимые символы для YooKassa от user_id={user_id}: {email}')
            await send_message_with_fallback(
                bot,
                user_id,
                escape_md('❌ Email содержит недопустимые символы. Используйте только буквы, цифры, точку, дефис и @.', version=2),
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='🔙 Назад к пакетам', callback_data='subscribe')]]),
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
    ...
    try:
        ...
        payment_url, payment_id = await create_payment_link(user_id, email, payment_amount, payment_description, bot_username)
        ...
        text_parts = [
            '💳 Оплата пакета\n',
            f'✨ Вы выбрали: {escape_md(payment_description, version=2)}{escape_md(bonus_text, version=2)}\n',
        ...
        logger.info(f'Платежная ссылка отправлена для user_id={user_id}: {payment_url}, payment_id={payment_id}')
        ...
        await send_message_with_fallback(
            bot,
            user_id,
            escape_md('❌ Ошибка при сохранении email. Попробуйте снова или обратитесь в поддержку: @AXIDI_Help', version=2),
            reply_markup=await create_subscription_keyboard(),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        await state.update_data(user_id=user_id)
...
async def handle_email_change_input(message: Message, state: FSMContext, email: str) -> None:
    ...
    try:
        ...
        await message.answer(
            ...
            reply_markup=await create_user_profile_keyboard(user_id, message.bot),
            parse_mode=ParseMode.MARKDOWN_V2
        )
    except Exception as e:
...
async def handle_admin_text(message: Message, state: FSMContext) -> None:
    ...
    try:
        from handlers.admin_panel import get_bot_summary_stats
        stats_text = await get_bot_summary_stats()
        # Делаем одну строчку длинной для лучшего отображения кнопок
        stats_text = stats_text.replace(
            '📊 Общая статистика бота:',
            '📊 Общая статистика бота ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━:'
        )
    except Exception:
        stats_text = '''🛠 Админ-панель:

📊 Общая статистика бота ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━:
👥 Всего пользователей: загрузка...
💰 Оплатили: загрузка...
💳 Всего платежей: загрузка...

🎭 Аватары:
🖼 Всего создано: загрузка...
✅ Успешных: загрузка...

📈 Активность:
🔥 Генераций за 30 дней: загрузка...

📅 За сегодня:
🆕 Новых пользователей: загрузка...
💳 Оплат: загрузка...
📝 Подписок: загрузка...
...
async def handle_admin_chat_message(bot, state: FSMContext, target_user_id: int, text: str, media_type: str = None, media_id: str = None) -> None:
...
async def handle_photo(message: Message, state: FSMContext) -> None:
    ...
    # Проверяем, заблокирован ли пользователь, и автоматически разблокируем если он снова пишет
    if await is_user_blocked(user_id):
        logger.info(f'Заблокированный пользователь user_id={user_id} снова отправил фото - автоматически разблокируем')
        try:
            await block_user_access(user_id, block=False, block_reason='Автоматическая разблокировка: пользователь снова отправил фото')
            logger.info(f'Пользователь user_id={user_id} успешно разблокирован')
