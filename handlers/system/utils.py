...
from handlers.utils import safe_escape_markdown as escape_md, send_message_with_fallback, safe_answer_callback, anti_spam, smart_message_send
from database import is_user_blocked, get_user_payments, check_database_user
...
async def handle_utils_callback(query: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        if callback_data == "back_to_menu":
            ...
            from handlers.user.commands import help_command
            ...
            await handle_check_training_callback(query, state, user_id)
        else:
            logger.error(f"Неизвестный callback_data: {callback_data} для user_id={user_id}")
            await smart_message_send(
                query,
                text=escape_md("❌ Неизвестное действие. Попробуйте снова или обратитесь в поддержку.", version=2),
        ...
        await smart_message_send(
            query,
            text=escape_md("❌ Произошла ошибка. Попробуйте снова или обратитесь в поддержку.", version=2),
            reply_markup=await create_main_menu_keyboard(user_id),
            parse_mode=ParseMode.MARKDOWN_V2
        )
...
async def handle_faq_topic_callback(query: CallbackQuery, state: FSMContext, user_id: int, topic: str) -> None:
    ...
    await smart_message_send(
        query, text=escaped_text, reply_markup=keyboard, parse_mode=ParseMode.MARKDOWN_V2
    )
...
async def handle_check_training_callback(query: CallbackQuery, state: FSMContext, user_id: int) -> None:
    """Статус всех аватаров пользователя - callback версия."""
    from database import get_user_trainedmodels, get_active_trainedmodel
    from keyboards import create_avatar_selection_keyboard
    
    try:
        # Получаем данные об аватарах
        ...
        if not trained_models:
            text = escape_md("🚫 У вас пока нет аватаров в обучении или готовых.", version=2) + "\n\n" + \
                   escape_md("Хочешь создать новый? 🛠", version=2) + "\n" + \
                   escape_md("Нажми 'Личный кабинет' → 'Создать аватар'!", version=2)
        ...
        # Формируем текст со статусом аватаров
        response_text = escape_md("🎭 Статус всех ваших аватаров:\n\n", version=2)
        ...
        response_text += escape_md("Выберите активный аватар нажав на нужную кнопку ниже!", version=2)
        ...
        logger.error(f"Ошибка в handle_check_training_callback для user_id={user_id}: {e}", exc_info=True)
        await smart_message_send(
            query,
            text=escape_md("❌ Ошибка получения статуса аватаров. Попробуйте позже.", version=2),
            reply_markup=await create_avatar_selection_keyboard(user_id),
            parse_mode=ParseMode.MARKDOWN_V2
        )