...
async def dev_test_tariff(message: Message, state: FSMContext) -> None:
    """Показывает тестовый тариф только главному разработчику."""
    user_id = message.from_user.id

    # Проверяем, что это главный разработчик
    if user_id != 5667999089:
        await message.answer("❌ Команда доступна только главному разработчику.")
        return
    ...
    if not test_tariff:
        await message.answer("❌ Тестовый тариф не найден в конфигурации.")
        return
...
🔗 Это специальный тариф для проверки работы платежной системы.""", version=2)
    ...
    logger.info(f"Тестовый тариф показан главному разработчику user_id={user_id}")
...
async def start(message: Message, state: FSMContext) -> None:
    ...
    logger.info(f"Пользователь {user_id} ({username}) запустил бота. Полный текст: {full_text}")
    ...
    if utm_source:
        logger.info(f"User {user_id} came with UTM source: {utm_source}")
        ...
        for arg in args:
            ...
            if arg.startswith("ref_"):
                try:
                    ...
                    logger.warning(f"Invalid referral link format: {arg}")
                    referrer_id = None
    ...
    if not is_notified and user_id not in ADMIN_IDS:
        ...
        try:
            ...
            logger.error(f"Ошибка отправки уведомления о новом пользователе через систему мониторинга: {e}")
...
async def addcook(message: Message, state: FSMContext) -> None:
    """Команда для добавления печенек пользователю (только админы)"""
    user_id = message.from_user.id
    ...
    if len(args) != 2:
        await message.answer(
            escape_md("❌ Неверный формат команды.\nИспользуйте: /addcook user_id количество\nПример: /addcook 123456 30", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    try:
        ...
        if amount <= 0:
            await message.answer(
                escape_md("❌ Количество печенек должно быть положительным числом.", version=2),
                parse_mode=ParseMode.MARKDOWN_V2
            )
            return
        ...
        await message.answer(
            escape_md("❌ Неверный формат. user_id и количество должны быть числами.", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    if not target_user_info:
        await message.answer(
            escape_md(f"❌ Пользователь с ID {target_user_id} не найден в базе данных.", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    confirm_text = escape_md(f"""🍪 Подтверждение добавления печенек

👤 Пользователь: {display_name} {username_display}
🆔 ID: {target_user_id}
📊 Текущий баланс: {current_cookies} печенек
➕ Добавить: {amount} печенек
📊 Станет: {current_cookies + amount} печенек

Подтвердить операцию?""", version=2)

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_addcook_{target_user_id}_{amount}"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_admin_operation")
        ]
    ])
    ...
    logger.info(f"Админ {user_id} запросил добавление {amount} печенек пользователю {target_user_id}")

async def delcook(message: Message, state: FSMContext) -> None:
    """Команда для удаления печенек у пользователя (только админы)"""
    user_id = message.from_user.id
    ...
    if len(args) != 2:
        await message.answer(
            escape_md("❌ Неверный формат команды.\nИспользуйте: /delcook user_id количество\nПример: /delcook 123456 10", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    confirm_text = escape_md(f"""🍪 Подтверждение удаления печенек

👤 Пользователь: {display_name} {username_display}
🆔 ID: {target_user_id}
📊 Текущий баланс: {current_cookies} печенек
➖ Удалить: {amount} печенек
📊 Станет: {current_cookies - amount} печенек
...
Подтвердить операцию?""", version=2)
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_delcook_{target_user_id}_{amount}"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_admin_operation")
        ]
    ])
    ...
    logger.info(f"Админ {user_id} запросил удаление {amount} печенек у пользователя {target_user_id}")

async def addnew(message: Message, state: FSMContext) -> None:
    """Команда для добавления 1 аватара пользователю (только админы)"""
    user_id = message.from_user.id
    ...
    if len(args) != 1:
        await message.answer(
            escape_md("❌ Неверный формат команды.\nИспользуйте: /addnew user_id\nПример: /addnew 123456", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    try:
        ...
        await message.answer(
            escape_md("❌ Неверный формат. user_id должен быть числом.", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    confirm_text = escape_md(f"""🎭 Подтверждение добавления аватара

👤 Пользователь: {display_name} {username_display}
🆔 ID: {target_user_id}
📊 Текущий баланс: {current_avatars} аватаров
➕ Добавить: 1 аватар
📊 Станет: {current_avatars + 1} аватаров

Подтвердить операцию?""", version=2)

    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_addnew_{target_user_id}"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_admin_operation")
        ]
    ])
    ...
    logger.info(f"Админ {user_id} запросил добавление 1 аватара пользователю {target_user_id}")

async def delnew(message: Message, state: FSMContext) -> None:
    """Команда для удаления 1 аватара у пользователя (только админы)"""
    user_id = message.from_user.id
    ...
    if len(args) != 1:
        await message.answer(
            escape_md("❌ Неверный формат команды.\nИспользуйте: /delnew user_id\nПример: /delnew 123456", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    if current_avatars <= 0:
        await message.answer(
            escape_md(f"❌ У пользователя нет аватаров для удаления.\nТекущий баланс: {current_avatars} аватаров", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    confirm_text = escape_md(f"""🎭 Подтверждение удаления аватара
...
Подтвердить операцию?""", version=2)
    ...
    keyboard = InlineKeyboardMarkup(inline_keyboard=[
        [
            InlineKeyboardButton(text="✅ Подтвердить", callback_data=f"confirm_delnew_{target_user_id}"),
            InlineKeyboardButton(text="❌ Отмена", callback_data="cancel_admin_operation")
        ]
    ])
    ...
    logger.info(f"Админ {user_id} запросил удаление 1 аватара у пользователя {target_user_id}")

async def user_id_info(message: Message, state: FSMContext) -> None:
    """Команда для получения информации о пользователе по ID или username (только админы)"""
    user_id = message.from_user.id
    ...
    if len(args) != 1:
        await message.answer(
            escape_md("❌ Неверный формат команды.\nИспользуйте: /id user_id или /id @username\nПример: /id 123456 или /id @username", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    if search_param.isdigit():
        target_user_id = int(search_param)
        target_user_info = await get_user_info(target_user_id)
    else:
        # Попытка найти по username
        target_user_info = await get_user_by_username(search_param)
        if target_user_info:
            target_user_id = target_user_info['user_id']

    if not target_user_info:
        await message.answer(
            escape_md(f"❌ Пользователь '{search_param}' не найден в базе данных.", version=2),
            parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    ...
    display_name = target_user_info.get('first_name') or target_user_info.get('username') or f"ID {target_user_info['user_id']}"
    username_display = f"(@{target_user_info.get('username')})" if target_user_info.get('username') else ""
    email_display = target_user_info.get('email') or "Не указан"

    # Основная информация
    info_text = escape_md(f"""👤 Информация о пользователе

🏷 Имя: {display_name} {username_display}
🆔 ID: {target_user_info['user_id']}
📧 Email: {email_display}
📅 Регистрация: {target_user_info.get('created_at', 'Неизвестно')}

💰 Баланс:
  • 🍪 Печеньки: {target_user_info.get('generations_left', 0)}
  • 🎭 Аватары: {target_user_info.get('avatar_left', 0)}

🔒 Статус: {'🚫 Заблокирован' if target_user_info.get('is_blocked') else '✅ Активен'}""", version=2)

    # Получаем статистику генераций
    try:
        gen_stats = await get_user_generation_stats(target_user_info['user_id'])
        if gen_stats:
            info_text += escape_md(f"\n\n📊 Статистика генераций:", version=2)
            type_names = {
            ...
            for gen_type, count in gen_stats.items():
                info_text += escape_md(f"\n  • {type_names.get(gen_type, gen_type)}: {count}", version=2)
    except Exception as e:
        logger.error(f"Ошибка получения статистики генераций для пользователя {target_user_info['user_id']}: {e}")

    # Получаем информацию об аватарах
    try:
        avatars = await get_user_trainedmodels(target_user_info['user_id'])
        if avatars:
            info_text += escape_md(f"\n\n🎭 Аватары ({len(avatars)}):", version=2)
            active_model_data = await get_active_trainedmodel(target_user_info['user_id'])
            active_avatar_id = active_model_data[0] if active_model_data else None

            for i, avatar_tuple in enumerate(avatars[:5]):  # Показываем только первые 5
                if len(avatar_tuple) >= 9:
                    ...
                    active_mark = " (Активный ✨)" if avatar_id == active_avatar_id and status == 'success' else ""
                    info_text += escape_md(f"\n  • {name_display}: {status_icon} {status}{active_mark}", version=2)

            if len(avatars) > 5:
                info_text += escape_md(f"\n  • ... и еще {len(avatars) - 5} аватаров", version=2)
    except Exception as e:
        logger.error(f"Ошибка получения информации об аватарах для пользователя {target_user_info['user_id']}: {e}")
    ...
    logger.info(f"Пользователь {user_id} запросил информацию о пользователе {target_user_info['user_id']}")
