# handlers/admin/visualization.py
...
async def show_visualization(query: CallbackQuery, state: FSMContext) -> None:
    ...
    keyboard = [
        [InlineKeyboardButton(text="📈 Платежи", callback_data="visualize_payments")],
        [InlineKeyboardButton(text="📊 Регистрации", callback_data="visualize_registrations")],
        [InlineKeyboardButton(text="📸 Генерации", callback_data="visualize_generations")],
        [InlineKeyboardButton(text="🔙 Админ-панель", callback_data="admin_panel")]
    ]
    await send_message_with_fallback(
        query.bot, user_id, text,
        reply_markup=InlineKeyboardMarkup(inline_keyboard=keyboard), parse_mode=ParseMode.MARKDOWN
    )
...
async def visualize_payments(query: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        if not any(amounts):
            ...
            await send_message_with_fallback(
                query.bot, user_id, text,
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="🔙 К визуализации", callback_data="admin_visualization")]]),
        ...
        reply_markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔙 К визуализации", callback_data="admin_visualization")],
            [InlineKeyboardButton(text="🏠 Админ-панель", callback_data="admin_panel")]
        ])
        await send_message_with_fallback(
            query.bot, user_id, text,
...
async def handle_activity_stats(query: CallbackQuery, state: FSMContext, start_date: str, end_date: str) -> None:
    ...
    try:
        ...
        if not stats:
            ...
            await send_message_with_fallback(
                query.bot, user_id, text,
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton("🔙 К активности", callback_data="admin_activity_stats")]]),
                parse_mode=ParseMode.MARKDOWN
            )
            return
        ...
        reply_markup = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📊 Повторить запрос", callback_data="admin_activity_stats")],
...
async def handle_activity_dates_input(message: Message, state: FSMContext) -> None:
    ...
    try:
        ...
        await state.update_data(awaiting_activity_dates=True)
        await message.answer(
            text, reply_markup=InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton("🔙 К активности", callback_data="admin_activity_stats")]]),
            parse_mode=ParseMode.MARKDOWN
        )
        await state.set_state(BotStates.AWAITING_ACTIVITY_DATES)
    except Exception as e:
...
async def cancel(message: Message, state: FSMContext) -> None:
    ...
    reply_markup = await create_admin_keyboard() if user_id in ADMIN_IDS else await create_admin_user_actions_keyboard(user_id)
    await message.answer(
