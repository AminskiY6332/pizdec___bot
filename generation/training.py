...
async def start_training(message: Message, state: FSMContext) -> None:
    ...
    async with TempFileManager() as temp_manager:
        try:
            ...
            await log_generation(user_id, 'train_flux', TRAINER_VERSION, units_generated=1, style='training', ratio=None)
...
async def check_training_status(bot: Bot, data: Dict[str, any]) -> None:
    ...
    try:
        ...
        if training_status == 'succeeded':
            ...
            # Отправляем немедленное уведомление главному разработчику
            try:
                from monitoring_system import BotMonitoringSystem
                monitoring = BotMonitoringSystem(bot)
                await monitoring.send_immediate_critical_notification("avatar_trained", {
                    'user_id': user_id,
                ...
                logger.error(f"Ошибка отправки немедленного уведомления об обучении аватара: {e}")

            # Отправляем обычные уведомления всем админам
