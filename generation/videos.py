...
async def generate_video(message: Message, state: FSMContext, task_id: int = None, prediction_id: str = None):
    ...
    async with TempFileManager() as temp_manager:
        try:
            ...
            # Логируем генерацию видео с style и ratio
            ...
            await log_user_action_with_style_ratio(
                ...
                details={
                    ...
                    'model_id': replicate_video_model_id,
                    'prompt': prompt[:100],  # Ограничиваем длину промпта
                    'task_id': task_id
                },
                style=style_for_log,
                ratio="16:9"  # Для видео всегда 16:9
            )
        ...
        if current_replicate_status == 'succeeded':
            ...
            if video_url:
                try:
                    ...
                    if model_key:
                        await log_generation(user_id, generation_type, model_key, units_generated=1, style=style_name, ratio="9:16")
