...
async def send_photo_with_retry(bot: Bot, chat_id: int, photo: FSInputFile, caption: str = None, reply_markup=None, parse_mode=None) -> Message:
    ...
    try:
        ...
        error_message = str(e)
        logger.error(f"Запрещено отправлять фото в chat_id={chat_id}: {error_message}")
...
async def send_media_group_with_retry(bot: Bot, chat_id: int, media: list):
    ...
    try:
        ...
        error_message = str(e)
        logger.error(f"Запрещено отправлять группу медиа в chat_id={chat_id}: {error_message}")
        ...
        logger.error(f"Ошибка отправки группы медиа для chat_id={chat_id}: {e}", exc_info=True)
        raise
...
async def send_video_with_retry(bot: Bot, chat_id: int, video, caption: str = None, reply_markup=None, parse_mode=None):
    ...
    try:
        ...
        error_message = str(e)
        logger.error(f"Запрещено отправлять видео в chat_id={chat_id}: {error_message}")
        ...
        logger.error(f"Ошибка отправки видео для chat_id={chat_id}: {e}", exc_info=True)
        raise
